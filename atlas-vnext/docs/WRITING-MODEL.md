...
3. Kept via contracts/behaviour: immutable version conflict (`stale_revision` / `VERSION_CONFLICT`), craft rules as composed Behaviour, selected-file grounding, bounded novel context, creative lineage distinct from evidential provenance.
...
6. Findings-only operations (`critique`, `continuity_check`) record findings without silently rewriting the manuscript.

See [CASPA-WRITING.md](./CASPA-WRITING.md) and [CASPA-NOVEL-MACHINE.md](./CASPA-NOVEL-MACHINE.md).
