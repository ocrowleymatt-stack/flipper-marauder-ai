    ...
    >
      ...
      <div className={`layout ${navOpen ? 'nav-open' : ''} ${diagnosticsOpen ? 'diagnostics-open' : ''}`}>
        <nav className="nav" aria-label="Workspace">
          <button type="button" className="primary new-conversation" onClick={() => void onCreateConversation()} disabled={projectsAvailable && !projectId}>
            New conversation
          </button>
          <form className="stack compact-form" onSubmit={(event) => void onCreateProject(event)}>
            ...
            <input
              ...
              autoComplete="off"
            />
            <button type="submit" className="ghost compact" disabled={!projectName.trim()}>
              Create project
            </button>
          </form>
          ...
          <div className="nav-section">
            ...
            <ul className="plain">
              <li>
                <button type="button" className={surface === 'files' ? 'active' : ''} onClick={() => openSurface('files')} disabled={!projectId}>
                  Files
                </button>
              </li>
              <li>
              ...
              <li>
                <button type="button" className={surface === 'writing' ? 'active' : ''} onClick={() => openSurface('writing')} disabled={!projectId}>
                  Caspa
                </button>
              </li>
            </ul>
        ...
        {surface === 'writing' && projectId ? (
          ...
          <main className="workspace" aria-label="Conversation">
            <header className="thread-header">
              ...
              <p className={`pill ${busy ? 'running' : latestExecution?.status ?? 'idle'}`} aria-live="polite">
                {runLabel}
              </p>
            </header>
            <section className="messages" aria-label="Messages" data-testid="conversation-thread">
              {!snapshot || snapshot.messages.length === 0 ? (
                ...
                snapshot.messages.map((message) => (
                  <article key={message.id} className={`message ${message.role}`} data-role={message.role}>
              ...
              <div className="composer-box">
                ...
                <div className="composer-actions">
                  {busy ? (
                    ...
                    <button className="send" type="submit" disabled={!draft.trim() || (projectsAvailable && !projectId)}>
                      Send
                    </button>
                  )}
                </div>
    ...
    <main className="workspace" aria-label="Files">
      ...
      <section className="estate-body">
        ...
        <form className="stack" onSubmit={(event) => void onUpload(event)}>
          ...
          <input id="file-path" value={fileDraft.path} onChange={(event) => setFileDraft((current) => ({ ...current, path: event.target.value }))} />
          <label htmlFor="file-text">Text</label>
          <textarea id="file-text" value={fileDraft.text} onChange={(event) => setFileDraft((current) => ({ ...current, text: event.target.value }))} rows={6} />
          <button type="submit" className="primary" disabled={!projectReady || !fileDraft.text.trim()}>
            Add file
          </button>
        </form>
        {files.length === 0 ? (
