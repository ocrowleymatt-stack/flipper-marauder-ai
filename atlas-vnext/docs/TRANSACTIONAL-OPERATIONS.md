...
1. **Idempotency keys** on job enqueue and event append.
2. **Explicit state transitions** — see `JOB_TRANSITIONS` in `platform/jobs`. Illegal transitions throw. Terminal job states are immutable except admin recovery.
...
6. **Retry safety** — execution may retry a candidate only **before** visible assistant output; tool calls are buffered until the stream completes.
7. **Audit / provenance** rows are append-only metadata (no blobs).
