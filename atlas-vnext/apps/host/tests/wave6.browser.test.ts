...
async function startUi() {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'atlas-wave6-ui-'));
...
describe.skipIf(!enabled)('Wave 6 Music recovery', { timeout: 300_000 }, () => {
  it('A/B/C/D/I: Ask Atlas composes music, card plays, library reopens, revision, writing still routes', async () => {
    ...
    try {
      ...
      await page.getByTestId('new-project-name').fill('Harbour score');
      ...
      await page.getByTestId('composer-draft').fill('Compose a 30-second piano piece in A minor, 90 BPM.');
      await page.getByTestId('composer-send').click();
      await page.getByTestId('result-card-music').waitFor({ timeout: 90_000 });
      expect(await page.getByTestId('result-card-music').innerText()).toMatch(/Revision/);
      expect(await page.getByTestId('music-play').innerText()).toMatch(/Play/i);
      await page.getByTestId('music-play').click();
      await page.getByTestId('open-music').click();
      await page.getByTestId('music-panel').waitFor();
      await page.getByTestId('back-to-conversation').click();
      await page.getByTestId('conversation-thread').waitFor();

      await page.getByTestId('composer-draft').fill('Make it slower.');
      await page.getByTestId('composer-send').click();
      await page.getByTestId('result-card-music').nth(1).waitFor({ timeout: 90_000 });
      expect(await page.getByTestId('result-card-music').nth(1).innerText()).toMatch(/Revision 2/);
      ...
      await page.getByTestId('music-panel').waitFor({ timeout: 15_000 });
      await page.getByTestId('back-to-conversation').click();

      await page.reload({ waitUntil: 'domcontentloaded' });
      await page.getByTestId('workbench-shell').waitFor({ timeout: 30_000 });
      await page.getByTestId('result-card-music').first().waitFor({ timeout: 15_000 });
      expect(await page.getByTestId('empty-conversation').count()).toBe(0);
      await page.screenshot({ path: join(shotDir, '14-wave6-desktop.png'), fullPage: true });
  ...
  it('J: 390 viewport keeps Music as a skill under Ask Atlas', async () => {
    const { url } = await startUi();
    ...
    try {
      ...
      expect(await page.getByTestId('empty-conversation').innerText()).toMatch(/Ask Atlas/);
      expect(await page.getByTestId('nav-dungeons').innerText()).toMatch(/Music/i);
      await page.getByTestId('new-project-name').fill('Mobile score');
      await page.getByTestId('create-project').click();
      await page.getByTestId('composer-draft').waitFor({ timeout: 20_000 });
      await page.getByTestId('composer-draft').fill('Write a song about rain');
      await page.getByTestId('composer-send').click();
      await page.getByTestId('result-card-music').waitFor({ timeout: 90_000 });
      await page.getByTestId('open-music').click();
      await page.getByTestId('music-panel').waitFor();
      ...
      await page.screenshot({ path: join(shotDir, '15-wave6-mobile.png'), fullPage: true });
    } finally {
      await browser.close();
    }
  });
